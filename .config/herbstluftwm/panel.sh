...
#font='xft:NotoColorEmoji:pixelsize=12:antialias=true:autohint=true'
...

{
    ...
    while true ; do
        ...
        date +$'date\t^fg(#BABABA)%H:%M^fg(#00FF00), %Y-%m-%d ^fg(#BABABA)%a'
        sleep 1 || break
        ...
        for i in "${tags[@]}" ; do
            case ${i:0:1} in
                '#')
                    ...
                    ;;
                '+')
                    echo -n "^bg(#FFFFFF)^fg(#000000)"
                    ;;
                ':')
                    echo -n "^bg(#ffffff)^fg(#000000)"
                    ;;
                '!')
                    echo -n "^bg(#FF0675)^fg(#000000)"
                    ;;
                *)
                    echo -n "^bg()^fg(#FFFFFF)"
                    #continue # hide that showed workspace
                    ;;
            esac
            if [ -n "$dzen2_svn" ] ; then
